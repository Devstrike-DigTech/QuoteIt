...
export class AuthService {
  ...
  public async register(user: RegisterDTO) {
    ...
    await this.UserRepository.create(
      {},
      {
        ...user,
        password,
        fada_id: this.generateRandomFadaId(),
        phone: formatted_phone_or_unverified_user,
      },
    );
  ...
  public async reset_password(
    ...
    if (!(await Cryptography.verify(hashedOtp as string, otp)))
  ...
  public async refreshToken(
    ...
    if (!foundToken) {
      const decodedUser = await this.JwtService.verify(refresh_token, {
        secret: REFRESH_TOKEN_SECRET,
      });
      if (decodedUser instanceof JsonWebTokenError)
        throw new ForbiddenException();

      await this.AuthMgtRepository.model.deleteMany({
        user_id: decodedUser.id,
      });
    }

    //2) verify refresh token
    const ExpiredOrInvalidToken = await this.JwtService.verify(refresh_token, {
      secret: REFRESH_TOKEN_SECRET,
    });
    ...
    if (
      ExpiredOrInvalidToken instanceof JsonWebTokenError ||
      ExpiredOrInvalidToken.id != foundToken.user_id
    ) {
      throw new UnauthorizedException();
    }
